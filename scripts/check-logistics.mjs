...
assert.equal(truck.state().x,parked.x);assert.equal(truck.state().y,parked.y);assert.equal(truck.state().phase,'loading');
assert.equal(truck.state().doors,1,'A requested departure starts by closing the open doors');
animation.currentTime=28799;truck.update(false);assert.equal(truck.state().doors,0,'Doors close before departure');
...
assert.deepEqual(histories.get('pallet-1'),['truck-in','aisle-truck','blue-buffer','aisle-truck','rack-ground','shuttle','rack-upper','shuttle','green-buffer','shuttle','truck-out']);
